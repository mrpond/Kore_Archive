...
# パッチサーバに接続する為にHTTP::Liteモジュールをロードする
use HTTP::Lite;

srand(time());

$versionText = "***Kore 0.90.42 - Ragnarok Online Bot - http://ro.L33T.ca***\n\n";
print $versionText;

addParseFiles("control/autosell.txt", \%autosell, \&parseDataFile_lc);
...
addParseFiles("control/warperauth.txt", \%partyWarperAuth, \&parseDataFile);
...
# 各種返答ファイルの読み込み
addParseFiles("control/sorryword.txt", \@sorryWord, \&parseDataFile3);
addParseFiles("control/angryword.txt", \@angryWord, \&parseDataFile3);
addParseFiles("control/judgebot.txt", \@judgebot, \&parseDataFile3);
addParseFiles("control/resbot.txt", \@resbot, \&parseDataFile3);
# 重要アイテムファイルの読み込み
...
if ($config{'partyWarperPassword'} eq 'x' x 10) {
	print "\nAuto-generating Party-Warper Password\n";
	configModify("partyWarperPassword", vocalString(8));
}
...
sub checkConnection {
	...
	} elsif ($conState == 1 && !($remote_socket && $remote_socket->connected()) && timeOut(\%{$timeout_ex{'master'}}) && !$conState_tries) {
	...
	if ($config{'local_port'} && time - $KoreStartTime > $config{'local_port'}) {
		$conState = 1;
		undef $conState_tries;
		$KoreStartTime = time;
		print "\nAuto-restarting!!\n\n";
...
.
		...
		if ($arg1 eq "") {
			...
			if ($chars[$config{'char'}]{'inventory'}[$arg1]{'type_equip'} == 1
				|| $chars[$config{'char'}]{'inventory'}[$arg1]{'type_equip'} == 256
				|| $chars[$config{'char'}]{'inventory'}[$arg1]{'type_equip'} == 512
...
.
			...
			for ($i = 0; $i < @equipment; $i++) {
				$display = $chars[$config{'char'}]{'inventory'}[$equipment[$i]]{'modified_name'};
...
.
		...
		if ($arg1 eq "" || $arg2 eq "") {
			...
			if ($arg1 - 1 >= @privMsgUsers) {
				...
				$lastpm{'msg'} = $arg2;
				$lastpm{'user'} = $privMsgUsers[$index - 1];
			}
		} else {
			if ($arg1 =~ /^%(\d*)$/) {
				$arg1 = $1;
...
.
		...
		for ($i = 0; $i < @playersID; $i++) {
			...
			format PLIST =
@<<< @<<< @<<< @<<<<<       @<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< @<<<<<<<< @<<<<<<<<<<
$i   $players{$playersID[$i]}{'pos_to'}{'x'} $players{$playersID[$i]}{'pos_to'}{'y'} $dPDist $name $sex_lut{$players{$playersID[$i]}{'sex'}} $jobs_lut{$players{$playersID[$i]}{'jobID'}}
.
			write;
		}
		print "---------------------------------\n";
...
.
	...
	} elsif ($switch eq "pw_auth") {
		($arg1, $arg2) = $input =~ /^[\s\S]*? ([\s\S]*) ([\s\S]*?)$/;
		if ($arg1 eq "" || ($arg2 ne "1" && $arg2 ne "0")) {
			print	"Syntax Error in function 'pw_auth' (partyWarper Authorize)\n"
				,"Usage: pw_auth <username> <flag>\n";
		} else {
			pw_auth($arg1, $arg2);
		}

	} elsif ($switch eq "pw_open") {
		pw_open(\$remote_socket);
		$timeout{'pw_open'}{'time'} = time;

	} elsif ($switch eq "pw_open2") {
		($arg1) = $input =~ /^[\s\S]*? ([\s\S]*)/;
		if ($arg1 ne "0" && $arg1 ne "1") {
			print	"Syntax Error in function 'pw_open2' (Open/Close Perpetual Portal)\n"
				,"Usage: pw_open2 <flag>\n";
		} else {
			pw_open2($arg1);
		}

	} elsif ($switch eq "pw_setup") {
		($arg1, $arg2) = $input =~ /^[\s\S]*? (\d+) (\d+)/;
		pw_setup($arg1, $arg2);
...
.
		...
		if ($arg1 eq "" || $arg2 eq "") {
			...
			if (!ai_getSkillUseType($skillsID[$arg1])) {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skillsID[$arg1]}{'ID'}, $arg3, 0, $monstersID[$arg2]);
			} else {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skillsID[$arg1]}{'ID'}, $arg3, 0, $monsters{$monstersID[$arg2]}{'pos_to'}{'x'}, $monsters{$monstersID[$arg2]}{'pos_to'}{'y'});
			}
		}
...
.
		...
		if ($arg1 eq "" || $arg2 eq "") {
			...
			if (!ai_getSkillUseType($skillsID[$arg1])) {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skillsID[$arg1]}{'ID'}, $arg3, 0, $playersID[$arg2]);
			} else {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skillsID[$arg1]}{'ID'}, $arg3, 0, $players{$playersID[$arg2]}{'pos_to'}{'x'}, $players{$playersID[$arg2]}{'pos_to'}{'y'});
			}
		}
		...
		if ($arg1 eq "") {
			...
			if (!ai_getSkillUseType($skillsID[$arg1])) {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skillsID[$arg1]}{'ID'}, $arg2, 0, $accountID);
			} else {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skillsID[$arg1]}{'ID'}, $arg2, 0, $chars[$config{'char'}]{'pos_to'}{'x'}, $chars[$config{'char'}]{'pos_to'}{'y'});
			}
		}
...
sub AI {
	...
	if (%cmd) {
		...
		if ($cmd{'user'} eq $chars[$config{'char'}]{'name'}) {
 		...
 		if ($cmd{'type'} eq "pm" || $cmd{'type'} eq "p" || $cmd{'type'} eq "g") {
			if ($config{'partyWarper'}) {
				$ai_v{'temp'}{'qm'} = quotemeta $config{'partyWarperPassword'};
				if ($cmd{'msg'} =~ /^$ai_v{'temp'}{'qm'}\b/) {
					if ($partyWarperAuth{$cmd{'user'}} == 1) {
						sendMessage(\$remote_socket, "pm", getResponse("pw_authF"), $cmd{'user'});
					} else {
						pw_auth($cmd{'user'}, 1);
						sendMessage(\$remote_socket, "pm", getResponse("pw_authS"), $cmd{'user'});
					}
				}
			}
		...
		$ai_v{'temp'}{'qm'} = quotemeta $config{'callSign'};
		if (($partyWarperAuth{$cmd{'user'}} >= 1 || $overallAuth{$cmd{'user'}} >= 1)
			&& ($cmd{'msg'} =~ /\b$ai_v{'temp'}{'qm'}\b/i || $cmd{'type'} eq "pm")
			&& $cmd{'msg'} =~ /\bheal\b/i) {
			$ai_v{'temp'}{'after'} = $';
			...
			if ($ai_v{'temp'}{'targetID'} eq "") {
				sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_healF1"), $cmd{'user'}) if $config{'verbose'};
				...
				while ($ai_v{'temp'}{'sp_used'} < $ai_v{'temp'}{'sp_needed'} && !$ai_v{'temp'}{'failed'}) {
					...
					if ($ai_v{'temp'}{'lv'} > 0) {
						...
						$ai_v{'temp'}{'skillCast'}{'castTime'} = 0;
						...
						sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_healF2"), $cmd{'user'}) if $config{'verbose'};
						$ai_v{'temp'}{'failed'} = 1;
					}
				}
				if (!$ai_v{'temp'}{'failed'}) {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_healS"), $cmd{'user'}) if $config{'verbose'};
				}
				foreach (@{$ai_v{'temp'}{'skillCasts'}}) {
					ai_skillUse($$_{'skill'}, $$_{'lv'}, $$_{'castTime'}, $$_{'ID'});
				}
			} else {
				sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_healF3"), $cmd{'user'}) if $config{'verbose'};
			}
		}

		if ($overallAuth{$cmd{'user'}} >= 1 || $partyWarperAuth{$cmd{'user'}} >= 1) {
		...
		$ai_v{'temp'}{'qm'} = quotemeta $config{'callSign'};
		if ($config{'partyWarper'} && ($cmd{'msg'} =~ /\b$ai_v{'temp'}{'qm'}\b/i || $cmd{'type'} eq "pm")) {
			if ($overallAuth{$cmd{'user'}} >= 1 
				&& $cmd{'msg'} =~ /\bauth/i 
				&& $cmd{'msg'} =~ /\bportal\b/i
				&& ($cmd{'msg'} =~ /\boff\b/i || $cmd{'msg'} =~ /\bdisable\b/i)) {
				if ($config{'partyWarperAuthorization'}) {
					pw_portalAuth(0);
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_portalAuthOffS"), $cmd{'user'}) if $config{'verbose'};
				} else {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_portalAuthOffF"), $cmd{'user'}) if $config{'verbose'};
				}
			} elsif ($overallAuth{$cmd{'user'}} >= 1
				&& $cmd{'msg'} =~ /\bauth/i 
				&& $cmd{'msg'} =~ /\bportal\b/i
				&& ($cmd{'msg'} =~ /on/i || $cmd{'msg'} =~ /\benable\b/i)) {
				if (!$config{'partyWarperAuthorization'}) {
					pw_portalAuth(1);
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_portalAuthOnS"), $cmd{'user'}) if $config{'verbose'};
				} else {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_portalAuthOnF"), $cmd{'user'}) if $config{'verbose'};
				}
			} elsif (($partyWarperAuth{$cmd{'user'}} >= 1 || $overallAuth{$cmd{'user'}} >= 1) 
				&& ($cmd{'msg'} =~ /\bsetup\b/i || $cmd{'msg'} =~ /\bmove\b/i)
				&& $cmd{'msg'} =~ /\bgate\b/i) {
				($ai_v{'temp'}{'x'}, $ai_v{'temp'}{'y'}) = $cmd{'msg'} =~ /(\d+)[\s\S]*?(\d+)/;
				if ($ai_v{'temp'}{'x'} ne "" || $ai_v{'temp'}{'y'} ne "") {
					ai_pw_setup($ai_v{'temp'}{'x'}, $ai_v{'temp'}{'y'});
				} else {
					ai_pw_setup($config{'partyWarper_portalX'}, $config{'partyWarper_portalY'});
				}
			} elsif (($partyWarperAuth{$cmd{'user'}} >= 1 || $overallAuth{$cmd{'user'}} >= 1)
				&& ($cmd{'msg'} =~ /\bwhere\b/i || $cmd{'msg'} =~ /\bstatus\b/i)
				&& $cmd{'msg'} =~ /\bgate\b/i) {
				if ($ai_v{'pw_perpetual'}) {
					$responseVars{'pw_portalStatus'} = "forever open";
				} elsif (!timeOut(\%{$timeout{'pw_open'}})) {
					$responseVars{'pw_portalStatus'} = "open";
				} else {
					$responseVars{'pw_portalStatus'} = "closed";
				}
				$responseVars{'pw_portalX'} = $config{'partyWarper_portalX'};
				$responseVars{'pw_portalY'} = $config{'partyWarper_portalY'};
				$responseVars{'pw_portalLoc'} = qq~$maps_lut{$config{"partyWarper_slot_$config{'partyWarper_slot_current'}"}} ($config{"partyWarper_slot_$config{'partyWarper_slot_current'}"})~;
				$timeout{'ai_thanks_set'}{'time'} = time;
				sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_statusS"), $cmd{'user'}) if $config{'verbose'};
			} elsif (($partyWarperAuth{$cmd{'user'}} >= 1 || $overallAuth{$cmd{'user'}} >= 1)
				&& $cmd{'msg'} =~ /\bO+P+E+N+\b/i 
				&& $cmd{'msg'} =~ /\bG+A+T+E+\b/i
				&& ($cmd{'msg'} =~ /keep/i  || $cmd{'msg'} =~ /leave/i)) {
				if (!$ai_v{'pw_setup'}) {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_open2OpenF1"), $cmd{'user'}) if $config{'verbose'};
					%{$ai_v{'pw_setup_exec'}} = %cmd if ai_pw_setup($config{'partyWarper_portalX'}, $config{'partyWarper_portalY'});
				} elsif (!$ai_v{'pw_perpetual'}) {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_open2OpenS"), $cmd{'user'}) if $config{'verbose'};
					pw_open2(1);
					$timeout{'ai_thanks_set'}{'time'} = time;
				} else {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_open2OpenF2"), $cmd{'user'}) if $config{'verbose'};
				}
			} elsif (($partyWarperAuth{$cmd{'user'}} >= 1 || !$config{'partyWarperAuthorization'} || $overallAuth{$cmd{'user'}} >= 1)
				&& $cmd{'msg'} =~ /\bO+P+E+N+\b/i 
				&& $cmd{'msg'} =~ /\bG+A+T+E+\b/i) {	
				if (!$ai_v{'pw_setup'}) {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_openF1"), $cmd{'user'}) if $config{'verbose'};
					%{$ai_v{'pw_setup_exec'}} = %cmd if ai_pw_setup($config{'partyWarper_portalX'}, $config{'partyWarper_portalY'});
				} elsif ($ai_v{'pw_perpetual'}) {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_openF2"), $cmd{'user'}) if $config{'verbose'};
				} elsif (!timeOut(\%{$timeout{'pw_open'}})) {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_openF3"), $cmd{'user'}) if $config{'verbose'};
				} else {
					pw_open(\$remote_socket);
					$timeout{'pw_open'}{'time'} = time;
					$timeout{'ai_thanks_set'}{'time'} = time;
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_openS"), $cmd{'user'}) if $config{'verbose'};
				}
			} elsif (($partyWarperAuth{$cmd{'user'}} >= 1 || $overallAuth{$cmd{'user'}} >= 1)
				&& $cmd{'msg'} =~ /\bC+L+O+S+E+\b/i 
				&& $cmd{'msg'} =~ /\bG+A+T+E+\b/i) {
				if ($ai_v{'pw_perpetual'}) {
					pw_open2(0);
					$timeout{'ai_thanks_set'}{'time'} = time;
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_open2CloseS"), $cmd{'user'}) if $config{'verbose'};
				} else {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_open2CloseF"), $cmd{'user'}) if $config{'verbose'};
				}
			} elsif (($partyWarperAuth{$cmd{'user'}} >= 1 || $overallAuth{$cmd{'user'}} >= 1)
				&& $cmd{'msg'} =~ /\blist/i 
				&& $cmd{'msg'} =~ /\bgate\b/i) {
				undef $responseVars{'pw_portalList'};
				$i = 0;
				while (1) {
					last if (!$config{"partyWarper_slot_$i"});
					$responseVars{'pw_portalList'} .= qq~$maps_lut{$config{"partyWarper_slot_$i"}} ($config{"partyWarper_slot_$i"})\\n~;
					$i++;
				}
				sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_listS"), $cmd{'user'}) if $config{'verbose'};
			} elsif (($partyWarperAuth{$cmd{'user'}} >= 1 || $overallAuth{$cmd{'user'}} >= 1)
				&& $cmd{'msg'} =~ /\bswitch\b/i 
				&& $cmd{'msg'} =~ /\bgate\b/i) {
				$ai_v{'temp'}{'after'} = $';
				$ai_v{'temp'}{'after'} =~ s/^\s+//;
				$ai_v{'temp'}{'after'} =~ s/\s+$//;
				@{$ai_v{'temp'}{'after'}} = split /\W/, $ai_v{'temp'}{'after'};
				undef $ai_v{'temp'}{'largest'};
				undef $ai_v{'temp'}{'largestSlot'};
				foreach $element (@{$ai_v{'temp'}{'after'}}) {
					$ai_v{'temp'}{'qm'} = quotemeta $element;
					$i = 0;
					while (1) {
						last if (!$config{"partyWarper_slot_$i"});
						if ($maps_lut{$config{"partyWarper_slot_$i"}} =~ /$ai_v{'temp'}{'qm'}/i) {
							if (length($element) > length($ai_v{'temp'}{'largest'})) {
								$ai_v{'temp'}{'largest'} = $element;
								$ai_v{'temp'}{'largestSlot'} = $i;
							}
						}
						$i++;
					}
				}
				if ($ai_v{'temp'}{'largest'}) {
					pw_switch($ai_v{'temp'}{'largestSlot'});
					$timeout{'ai_thanks_set'}{'time'} = time;
					$responseVars{'pw_portalLoc'} = qq~$maps_lut{$config{"partyWarper_slot_$config{'partyWarper_slot_current'}"}} ($config{"partyWarper_slot_$config{'partyWarper_slot_current'}"})~;
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_switchS"), $cmd{'user'}) if $config{'verbose'};
				} else {
					sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_switchF"), $cmd{'user'}) if $config{'verbose'};
				}
			}
		}
	}
	...
	if ($ai_seq[0] eq "sellAuto" && $ai_seq_args[0]{'done'}) {
		...
		if ($ai_v{'temp'}{'do_route'}) {
			...
			for ($i = 0; $i < @{$chars[$config{'char'}]{'inventory'}};$i++) {
				...
				if ($autosell{lc($chars[$config{'char'}]{'inventory'}[$i]{'name'})} ne ""
					&& $chars[$config{'char'}]{'inventory'}[$i]{'amount'} > $autosell{lc($chars[$config{'char'}]{'inventory'}[$i]{'name'})}) {
	...
	if (($ai_seq[0] eq "" || $ai_seq[0] eq "route") && timeOut(\%{$timeout{'ai_buyAuto'}})) {
	...
	##### RANDOM WALK #####
	if ($config{'route_randomWalk'} && $ai_seq[0] eq "" && %field && !$cities_lut{$field{'name'}.'.rsw'}) {
		do { 
		...
		while (1) {
			...
			if (percent_hp(\%{$chars[$config{'char'}]}) <= $config{"useSelf_item_$i"."_hp_upper"} && percent_hp(\%{$chars[$config{'char'}]}) >= $config{"useSelf_item_$i"."_hp_lower"}
				...
				if ($ai_v{'temp'}{'invIndex'} ne "") {
					...
					# ラグ時のアイテム連続使用
					if(!$config{'useSelf_item_continue'}){
						$config{'useSelf_item_continue'} = 1;
					}
					my $c;
					for($c = 0; $c < $config{'useSelf_item_continue'}; $c++){
	...
	if (binFind(\@ai_seq, "skill_use") eq "") {
		$i = 0;
		...
		if ($ai_v{'useSelf_skill_lvl'} > 0) {
			...
			if (!ai_getSkillUseType($skills_rlut{lc($ai_v{'useSelf_skill'})})) {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skills_rlut{lc($ai_v{'useSelf_skill'})}}{'ID'}, $ai_v{'useSelf_skill_lvl'}, $ai_v{'useSelf_skill_maxCasttime'}, $accountID);
			} else {
				ai_skillUse($chars[$config{'char'}]{'skills'}{$skills_rlut{lc($ai_v{'useSelf_skill'})}}{'ID'}, $ai_v{'useSelf_skill_lvl'}, $ai_v{'useSelf_skill_maxCasttime'}, $chars[$config{'char'}]{'pos_to'}{'x'}, $chars[$config{'char'}]{'pos_to'}{'y'});
			}
		}
	}
	...
	if ($ai_seq[0] eq "skill_use" && $ai_seq_args[0]{'suspended'}) {
		...
		$ai_seq_args[0]{'ai_skill_use_cast_giveup'}{'time'} += time - $ai_seq_args[0]{'suspended'};
	...
	if ($ai_seq[0] eq "skill_use") {
		if ($chars[$config{'char'}]{'sitting'}) {
			...
			|| (timeOut(\%{$ai_seq_args[0]{'ai_skill_use_giveup'}}) && (!$chars[$config{'char'}]{'time_cast'} || !$ai_seq_args[0]{'skill_use_cast_giveup'}{'timeout'}))
			|| ($ai_seq_args[0]{'skill_use_cast_giveup'}{'timeout'} && timeOut(\%{$ai_seq_args[0]{'skill_use_cast_giveup'}}))) {
			$ai_v{'skill_use_wasSitting'} = $ai_seq_args[0]{'skill_use_wasSitting'};
			shift @ai_seq;
			shift @ai_seq_args;
		}
	}
	...
	if (($ai_seq[0] eq "" || $ai_seq[0] eq "route" || $ai_seq[0] eq "route_getRoute" || $ai_seq[0] eq "route_getMapRoute" || $ai_seq[0] eq "follow" || $ai_seq[0] eq "mapLock" || $ai_seq[0] eq "sitAuto" || $ai_seq[0] eq "take" || $ai_seq[0] eq "items_gather" || $ai_seq[0] eq "items_take")
	...
	if ($ai_seq[0] eq "attack" && timeOut(\%{$ai_seq_args[0]{'ai_attack_giveup'}})) {
		...
		if ($monsters_old{$ai_v{'ai_attack_ID_old'}}{'dead'}) {
			...
			if ($config{'itemsTakeAuto'} && $monsters_old{$ai_v{'ai_attack_ID_old'}}{'dmgFromYou'} > 0) {
				ai_items_take($monsters_old{$ai_v{'ai_attack_ID_old'}}{'pos_to'}{'x'}, $monsters_old{$ai_v{'ai_attack_ID_old'}}{'pos_to'}{'y'});
			}
		} else {
			print "Target lost\n";
		}
		...
		if ($chars[$config{'char'}]{'sitting'}) {
			...
			|| !$config{'tankMode'})) {
			if ($ai_seq_args[0]{'dmgFromYou_last'} != $monsters{$ai_seq_args[0]{'ID'}}{'dmgFromYou'}) {
			...
			if ($ai_seq_args[0]{'attackMethod'}{'type'} eq "weapon" && timeOut(\%{$timeout{'ai_attack'}})) {
				...
				if (!ai_getSkillUseType($skills_rlut{lc($config{"attackSkillSlot_$ai_v{'ai_attack_method_skillSlot'}"})})) {
					ai_skillUse($chars[$config{'char'}]{'skills'}{$skills_rlut{lc($config{"attackSkillSlot_$ai_v{'ai_attack_method_skillSlot'}"})}}{'ID'}, $config{"attackSkillSlot_$ai_v{'ai_attack_method_skillSlot'}"."_lvl"}, $config{"attackSkillSlot_$ai_v{'ai_attack_method_skillSlot'}"."_maxCastTime"}, $ai_v{'ai_attack_ID'});
				} else {
					ai_skillUse($chars[$config{'char'}]{'skills'}{$skills_rlut{lc($config{"attackSkillSlot_$ai_v{'ai_attack_method_skillSlot'}"})}}{'ID'}, $config{"attackSkillSlot_$ai_v{'ai_attack_method_skillSlot'}"."_lvl"}, $config{"attackSkillSlot_$ai_v{'ai_attack_method_skillSlot'}"."_maxCastTime"}, $monsters{$ai_v{'ai_attack_ID'}}{'pos_to'}{'x'}, $monsters{$ai_v{'ai_attack_ID'}}{'pos_to'}{'y'});
				}
	...
	if ($ai_seq[0] eq "route" && @{$ai_seq_args[0]{'solution'}} && $ai_seq_args[0]{'index'} == @{$ai_seq_args[0]{'solution'}} - 1 && $ai_seq_args[0]{'solutionReady'}) {
		...
		if (@{$ai_seq_args[0]{'mapSolution'}} && $ai_seq_args[0]{'mapChanged'} && $field{'name'} eq $ai_seq_args[0]{'mapSolution'}[$ai_seq_args[0]{'mapIndex'}]{'dest'}{'map'}) {
			...
			undef $ai_seq_args[0]{'offRoute_moved'};
		}
		if (!@{$ai_seq_args[0]{'solution'}}) {
		...
		if ($ai_seq_args[0]{'mapChanged'}) {
			...
			#something moved us off route - try a simple move back first, then re-calc if failed again
			$ai_seq_args[0]{'offRoute_moved'} = 1;
		...
		} elsif ($ai_seq_args[0]{'offRoute_moved'} && $ai_seq_args[0]{'last_pos'}{'x'} == $chars[$config{'char'}]{'pos_to'}{'x'}
				&& $ai_seq_args[0]{'last_pos'}{'y'} == $chars[$config{'char'}]{'pos_to'}{'y'}) {

			print "Off route, recalculating route...\n";
	...
	if ($ai_seq[0] eq "route_getRoute" && ($ai_seq_args[0]{'done'} 
		...
		if ($ai_seq_args[0]{'start'}{'f'} eq "") {
			...
			$ai_seq_args[0]{'done'} = 1 if (ai_route_getMap(\%{$ai_seq_args[0]}, $ai_seq_args[0]{'start'}{'x'}, $ai_seq_args[0]{'start'}{'y'}) || ai_route_getMap(\%{$ai_seq_args[0]}, $ai_seq_args[0]{'dest'}{'x'}, $ai_seq_args[0]{'dest'}{'y'}));
			$ai_seq_args[0]{'start'}{'g'} = 0;
			$ai_seq_args[0]{'start'}{'h'} = distance($ai_seq_args[0]{'start'}, $ai_seq_args[0]{'dest'});
			$ai_seq_args[0]{'start'}{'f'} = $ai_seq_args[0]{'start'}{'g'} + $ai_seq_args[0]{'start'}{'h'};
			$ai_seq_args[0]{'start'}{'parent'} = 0;
	...
	if ($ai_seq[0] eq "route_getMapRoute" && ($ai_seq_args[0]{'done'} || ($ai_seq_args[0]{'time_giveup'}{'timeout'} && timeOut(\%{$ai_seq_args[0]{'time_giveup'}})))) {
		$timeout{'ai_route_calcRoute_cont'}{'time'} -= $timeout{'ai_route_calcRoute_cont'}{'timeout'};
		shift @ai_seq;
		shift @ai_seq_args;
	...
	if ($config{'itemsTakeAuto'} && $ai_seq[0] eq "items_take" && timeOut(\%{$ai_seq_args[0]{'ai_items_take_start'}})) {
		...
		foreach (@itemsID) {
			...
			if ($ai_v{'temp'}{'dist'} <= 3 && $items{$_}{'take_failed'} == 0) {
				$ai_v{'temp'}{'foundID'} = $_;
				last;
			}
		}
	...
	if (($ai_seq[0] eq "" || $ai_seq[0] eq "follow" || $ai_seq[0] eq "route" || $ai_seq[0] eq "route_getRoute" || $ai_seq[0] eq "route_getMapRoute") && $config{'itemsGatherAuto'} && !(percent_weight(\%{$chars[$config{'char'}]}) >= $config{'itemsMaxWeight'}) && timeOut(\%{$timeout{'ai_items_gather_auto'}})) {
		...
		foreach $item (@itemsID) {
			next if ($item eq "" || time - $items{$item}{'time_appear'} < $timeout{'ai_items_gather_start'}{'timeout'}
	...
	} elsif ($ai_seq[0] eq "sitting" && !$chars[$config{'char'}]{'sitting'} && timeOut(\%{$timeout{'ai_sit'}})) {
		sendSit(\$remote_socket);
		$timeout{'ai_sit'}{'time'} = time;
	}
	if ($ai_seq[0] eq "standing" && !$chars[$config{'char'}]{'sitting'}) {
		shift @ai_seq;
		shift @ai_seq_args;
		$timeout{'ai_sit'}{'time'} -= $timeout{'ai_sit'}{'timeout'};
	...
	if ($config{'partyWarper'}) {
		if ($ai_seq[0] eq "pw_setup1") {
			if ($chars[$config{'char'}]{'sitting'}) {
				sendStand(\$remote_socket);
			}
			sendWarpCast1(\$remote_socket, $config{'partyWarper_portalX'}, $config{'partyWarper_portalY'});
			$timeout{'pw_setup2'}{'time'} = time;
			shift @ai_seq;
			shift @ai_seq_args;
			unshift @ai_seq, "pw_setup2";
			unshift @ai_seq_args, {};
		} elsif ($ai_seq[0] eq "pw_setup2" && timeOut(\%{$timeout{'pw_setup2'}})) {
			sendSit(\$remote_socket);
			shift @ai_seq;
			shift @ai_seq_args;
			$ai_v{'pw_setup'} = 1;
			AI(\%{$ai_v{'pw_setup_exec'}}) if (%{$ai_v{'pw_setup_exec'}});
			undef %{$ai_v{'pw_setup_exec'}};
			look($chars[$config{'char'}]{'look'}{'body'}, $chars[$config{'char'}]{'look'}{'head'});
		}
		if ($ai_v{'pw_setup'} && $ai_v{'pw_perpetual'} == 1 && timeOut(\%{$timeout{'pw_open'}})) {
			pw_open(\$remote_socket);
			$timeout{'pw_open'}{'time'} = time;
		}
	}
	...
	if (percent_hp(\%{$chars[$config{'char'}]}) <= $config{'teleportAuto_hp'} && binFind(\@ai_seq, "mapLock") eq "" && ai_getAggressives()
	...
	if ($config{'teleportAuto_search'} && timeOut(\%{$timeout{'ai_teleport_search'}}) && binFind(\@ai_seq, "mapLock") eq "" && binFind(\@ai_seq, "attack") eq "" && binFind(\@ai_seq, "items_take") eq ""
	...
	if ($config{'teleportAuto_idle'} && binFind(\@ai_seq, "mapLock") eq "" && timeOut(\%{$timeout{'ai_teleport_idle'}}) && $ai_v{'ai_teleport_safe'}) {
		sendTeleport(\$remote_socket, "Random");
		$ai_v{'clear_aiQueue'} = 1;
		$timeout{'ai_teleport_idle'}{'time'} = time;
	}

	if ($config{'teleportAuto_portal'} && binFind(\@ai_seq, "mapLock") eq "" && timeOut(\%{$timeout{'ai_teleport_portal'}}) && $ai_v{'ai_teleport_safe'}) {
...
sub parseMsg {
	...
	if ($flagImportantItemsMode && (time - $startImportantTime) > $config{'timeCutImportant'}) {
		$flagImportantItemsMode = 0;
		$config{'attackAuto'} = $old_attack_mode;
	}
	
	if ($flagahi && time - $ahiTalk > 30) {
		$ahiTalk = time;
			sendMessage(\$remote_socket, "pm", "$config1", $Usename);
			sendMessage(\$remote_socket, "pm", "$config2", $Usename);
	}
...
.
		...
		$startTime_EXP = time;
		# HPR/SPRスキルハック
		if($config{'hprOnStart'}){
			sendAddSkillPoint(\$remote_socket, 4);
			print "HP回復スキルを覚えるパケットを送りました。\n";
		}
		if($config{'sprOnStart'}){
			sendAddSkillPoint(\$remote_socket, 9);
			print "SP回復スキルを覚えるパケットを送りました。\n";
		}
		$flagahi = 1;
		$ahiTalk = time;
		$msg_size = 11;
		...
		# 自動返答
		my $dPDist = distance(\%{$chars[$config{'char'}]{'pos_to'}}, \%{$players{$playersID[$useID]}{'pos_to'}});
		if($dPDist < $config{'distResponse'}){
			undef $resMsg;
			$resMsg = judgeRes($chatMsg, 'judgebot', 'resbot', $ID);
			if ($resMsg) {
				sleep(int(rand(3))+2);
		...
		sendMapLoaded(\$remote_socket);
		sendPartyShareEXP(\$remote_socket, 1) if($config{'shareOnMapChanged'});
		$msg_size = 22;
...
.
		...
		if ($fail == 0) {
			if ($invIndex eq "" || $itemSlots_lut{$ID} != 0) {
				...
				if ($chars[$config{'char'}]{'inventory'}[$invIndex]{'type_equip'}) {
					$chars[$config{'char'}]{'inventory'}[$invIndex]{'attribute'} = unpack("C1", substr($msg, $i + 10, 1));
			...
			if (!$chars[$config{'char'}]{'inventory'}[$invIndex]{'type_equip'}) {
				print "Item added to inventory: $chars[$config{'char'}]{'inventory'}[$invIndex]{'modified_name'} ($invIndex) x $amount - $itemTypes_lut{$chars[$config{'char'}]{'inventory'}[$invIndex]{'type'}}\n";
			} else {
				print "Item added to inventory: $chars[$config{'char'}]{'inventory'}[$invIndex]{'modified_name'} ($invIndex) x $amount - $itemTypes_lut{$chars[$config{'char'}]{'inventory'}[$invIndex]{'type'}} - $equipTypes_lut{$chars[$config{'char'}]{'inventory'}[$invIndex]{'type_equip'}}\n";
			}
			# 重要アイテムを拾ったらログに残し、攻撃モードを元に戻す
			if($config{'itemsImportantAuto'}){
		...
		for($i = 4; $i < $msg_size; $i+=10) {
			...
			if ($invIndex eq "") {
				...
				$display = ($items_lut{$chars[$config{'char'}]{'inventory'}[$invIndex]{'nameID'}} ne "")
					...
					: "Unknown ".$chars[$config{'char'}]{'inventory'}[$invIndex]{'nameID'};
				$chars[$config{'char'}]{'inventory'}[$invIndex]{'name'} = $display;
				$chars[$config{'char'}]{'inventory'}[$invIndex]{'modified_name'} = $display;
				print "Inventory: $chars[$config{'char'}]{'inventory'}[$invIndex]{'modified_name'} ($invIndex) x $chars[$config{'char'}]{'inventory'}[$invIndex]{'amount'} - $itemTypes_lut{$chars[$config{'char'}]{'inventory'}[$invIndex]{'type'}}\n" if $config{'debug'};
			}	
		}
		...
		for($i = 4; $i < $msg_size; $i+=20) {
			...
			if ($invIndex eq "") {
				...
				$chars[$config{'char'}]{'inventory'}[$invIndex]{'name'} = $display;
				modifingName(\%{$chars[$config{'char'}]{'inventory'}[$invIndex]});
				print "Inventory: $chars[$config{'char'}]{'inventory'}[$invIndex]{'modified_name'} ($invIndex) x $chars[$config{'char'}]{'inventory'}[$invIndex]{'amount'} - $itemTypes_lut{$chars[$config{'char'}]{'inventory'}[$invIndex]{'type'}} - $equipTypes_lut{$chars[$config{'char'}]{'inventory'}[$invIndex]{'type_equip'}}\n" if $config{'debug'};
			}
		}
		...
		# ヒールもらったら返答
		if ($config{'useThanks'} && $sourceID != $accountID && $targetID == $accountID && $skillID == 28) {
			sleep(2);
			if ($config{'useThanks'} == 1){
				sendMessage(\$remote_socket, "c", $config{'useThanksWord'});
			} else {
				sendEmotion(\$remote_socket, $config{'useThanksEmotion'});
			}
		}
		...
		# ワープポータル回避
		$type =  unpack("C1",substr($msg, 14, 1));
		if ($type eq 0x81 || $type eq 0x82) {
			print "Portal opened at $x, $y\n";
			if ($config{'avoidPortal'} && $x eq $chars[$config{'char'}]{'pos_to'}{'x'} && $y eq $chars[$config{'char'}]{'pos_to'}{'y'}) {
				sendTeleport(\$remote_socket, "Random");
			}
		}
		# アイスウォール検知
		if ($type eq 0x8d) {
			print "Ice Wall setup at $x, $y\n"; 
		}
...
#Cart Parses - chobit andy 20030102
		...
		for($i = 4; $i < $msg_size; $i+=20) {
			...
			print "Cart Item: $cart{'inventory'}{$ID}{'name'} ($cart{'inventory'}{$ID}{'binID'}) x 1\n" if ($config{'debug'} >= 1);
		}
		...
		print "Now using $skillsID_lut{$skillID}\n";
		sendSkillUse(\$remote_socket, $skillID, 1, $accountID);
		$msg_size = 39;
		...
		$type = unpack("C1",substr($msg, 2, 1));
		$ID = substr($msg, 3, 4);
		#just to make sure we have it in the hash for now
		if (!%{$pets{$ID}}) {
			binAdd(\@petsID, $ID);
			$pets{$ID}{'nameID'} = 0;
...
sub ai_pw_setup {
	...
	my $y = shift;
	if ($x !~ /^\d+$/ || $y !~ /^\d+$/) {
		$responseVars{'pw_setupErrorX'} = $x;
		$responseVars{'pw_setupErrorY'} = $y;
		sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_setupF1"), $cmd{'user'}) if $config{'verbose'};
		return 0;
	} elsif ($chars[$config{'char'}]{'pos'}{'x'} == $x && $chars[$config{'char'}]{'pos'}{'y'} == $y) {
		sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_setupF2"), $cmd{'user'}) if $config{'verbose'};
		return 0;
	} elsif ($chars[$config{'char'}]{'sp'} < 30) {
		$responseVars{'char_sp'} = $chars[$config{'char'}]{'sp'};
		sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_setupF3"), $cmd{'user'}) if $config{'verbose'};
		return 0;
	} else {
		pw_setup($x, $y);
		$timeout{'ai_thanks_set'}{'time'} = time;
		sendMessage(\$remote_socket, $cmd{'type'}, getResponse("pw_setupS"), $cmd{'user'}) if $config{'verbose'};
		return 1;
	}
}
...
sub ai_route_getRoute {
	...
	$args{'g_normal'} = 0.5;
	$args{'g_trace'} = 0.5;
	$args{'normal_giveup'} = 60;
	$args{'trace_minimumSteps'} = 1;
...
sub ai_route_searchStep {
	...
	my @subSuc;
	my $newg;
	my $found, $openFound, $closedFound;
	my $i;
	my $r_cur;
	my $result, $result2;
	...
	if ($$r_args{'tracing'}{'active'} && !@{$$r_args{'traceList'}}) {
		#failed trace!
		undef %{$$r_args{'tracing'}};
		undef @{$$r_args{'traceList'}};
		undef @{$$r_args{'traceListAll'}};
		$$r_args{'done'} = 1;
		return;
	}
	...
	if (!$$r_args{'tracing'}{'active'}) {
		$r_cur = shift @{$$r_args{'openList'}};
	} else {
		$r_cur = shift @{$$r_args{'traceList'}};
		$result = lineIntersection(\%{$$r_cur{'parent'}}, $r_cur, \%{$$r_args{'tracing'}{'lineStart'}}, \%{$$r_args{'tracing'}{'lineEnd'}});
		$result2 = lineIntersection(\%{$$r_args{'tracing'}{'lineStart'}}, \%{$$r_args{'tracing'}{'lineEnd'}}, \%{$$r_cur{'parent'}}, $r_cur);
		if ($result ne "" && $result2 ne "" && $result >= 0 && $result <= 1 && $result2 >= 0 && $result2 <= 1
			&& $$r_cur{'g'} - $$r_args{'tracing'}{'f_smallest'}{'g'} > $$r_args{'trace_minimumSteps'}*$$r_args{'g_trace'}) {
			$$r_cur{'g'} = 0;
			$$r_cur{'h'} = distance($r_cur, $$r_args{'dest'});
			$$r_cur{'f'} = $$r_cur{'g'} + $$r_cur{'h'};
			unshift @{$$r_args{'openList'}}, $r_cur;
			undef %{$$r_args{'tracing'}};
			undef @{$$r_args{'traceList'}};
			undef @{$$r_args{'traceListAll'}};
	...
	###check if finished
	if ($$r_args{'dest'}{'x'} == $$r_cur{'x'} && $$r_args{'dest'}{'y'} == $$r_cur{'y'}) {
		do {
			unshift @{$$r_args{'solutionList'}}, {%{$r_cur}};
			$r_cur = $$r_cur{'parent'} if (%{$$r_cur{'parent'}});
		} while ($r_cur != $$r_args{'start'});
		unshift @{$$r_args{'solutionList'}}, {%{$$r_args{'dest'}}};
		$$r_args{'done'} = 1;
		return;
	}
	...
	###check if tracing should be activated
	
	if (!$$r_args{'tracing'}{'active'}) {
		if (!$$r_args{'tracing'}{'f_first'} || $$r_cur{'f'} < $$r_args{'tracing'}{'f_smallest'}{'f'}) {
			$$r_args{'tracing'}{'f_first'} = 1;
			$$r_args{'tracing'}{'f_smallest'} = $r_cur;
			undef $$r_args{'tracing'}{'stuckSteps'};
		} else {
			$$r_args{'tracing'}{'stuckSteps'}++;
		}
	}

	if (!$$r_args{'tracing'}{'active'} && $$r_args{'tracing'}{'stuckSteps'} >= $$r_args{'normal_giveup'}) {

		#start tracing at node with smallest f
		$r_cur = $$r_args{'tracing'}{'f_smallest'};
		$result = $$r_cur{'parent'};
		undef $$r_cur{'parent'};
		ai_route_getSuccessors($r_args, $r_cur, \@successors, 0);
		$$r_cur{'parent'} = $result;
		%{$$r_args{'tracing'}{'lineStart'}} = %{$r_cur};
		%{$$r_args{'tracing'}{'lineEnd'}} = %{$$r_args{'dest'}};
		unshift @{$$r_args{'traceListAll'}}, $r_cur;
		$$r_args{'tracing'}{'active'} = 1;
	} else {
		ai_route_getSuccessors($r_args, $r_cur, \@successors, 0);
	}
	...
	if (!$$r_args{'tracing'}{'active'}) {
		...
		foreach $r_suc (@successors) {
			...
			undef $closedFound;
			for($i = 0; $i < @{$$r_args{'openList'}}; $i++) {
				if ($$r_suc{'x'} == $$r_args{'openList'}[$i]{'x'} && $$r_suc{'y'} == $$r_args{'openList'}[$i]{'y'}) {
					if ($newg >= $$r_args{'openList'}[$i]{'g'}) {
						$found = 1;
						}
					$openFound = $i;
			...
			undef $found;
			for($i = 0; $i < @{$$r_args{'closedList'}}; $i++) {
				if ($$r_suc{'x'} == $$r_args{'closedList'}[$i]{'x'} && $$r_suc{'y'} == $$r_args{'closedList'}[$i]{'y'}) {
					if ($newg >= $$r_args{'closedList'}[$i]{'g'}) {
						$found = 1;
					}
					$closedFound = $i;
			...
			$$r_suc{'g'} = $newg;
			$$r_suc{'h'} = distance($r_suc, $$r_args{'dest'});
			$$r_suc{'f'} = $$r_suc{'g'} + $$r_suc{'h'};
			$$r_suc{'parent'} = $r_cur;
			minHeapAdd(\@{$$r_args{'openList'}}, $r_suc, "f");
		}	
		...
		$newg = $$r_cur{'g'} + $$r_args{'g_trace'};
		foreach $r_suc (@successors) {
			undef @subSuc;
			$$r_suc{'parent'} = $r_cur;
			ai_route_getSuccessors($r_args, $r_suc, \@subSuc,0);
			ai_route_getDiagSuccessors($r_args, $r_suc, \@subSuc,0);
			if (@subSuc <= 6) {
				undef $found;
				for($i = 0; $i < @{$$r_args{'traceListAll'}}; $i++) {
					if ($$r_suc{'x'} == $$r_args{'traceListAll'}[$i]{'x'} && $$r_suc{'y'} == $$r_args{'traceListAll'}[$i]{'y'}) {
						if ($newg >= $$r_args{'traceListAll'}[$i]{'g'}) {
							$found = 1;
						}
						last;
					}
				...
				$$r_suc{'h'} = distance($r_suc, $$r_args{'dest'});
				$$r_suc{'f'} = $$r_suc{'g'} + $$r_suc{'h'};
				minHeapAdd(\@{$$r_args{'traceList'}}, $r_suc, "f");
				unshift @{$$r_args{'traceListAll'}}, $r_suc;
			}
		}
	}
}
...
sub ai_skillUse {
	...
	$args{'skill_use_cast_giveup'}{'time'} = time;
	$args{'skill_use_cast_giveup'}{'timeout'} = $casttime;
	if ($y eq "") {
...
sub aiRemove {
	...
	my $index;
	while (1) {
		$index = binRemoveAndShift(\@ai_seq, $ai_type);
		if ($index ne "") {
...
sub pw_open {
	...
	my $map;
	print "Portal opened\n";
	($map) = $config{"partyWarper_slot_$config{'partyWarper_slot_current'}"} =~ /([\s\S]*)\./;
	$map .= ".gat";
	sendWarpCast2($r_socket, $map);
}

sub pw_open2 {
	my $flag = shift;
	if ($flag) {
		print "Perpetual portal opened\n";
	} else {
		print "Closed perpetual portal to mid4\n";
	}
	$ai_v{'pw_perpetual'} = $flag;
}

sub pw_setup {
	my $x = shift;
	my $y = shift;
	unshift @ai_seq, "pw_setup1";
	unshift @ai_seq_args, {};
	if ($x =~ /^\d+$/ && $y =~ /^\d+$/  
		&& ($x != $config{'partyWarper_portalX'} || $y != $config{'partyWarper_portalY'})) {
		$config{'partyWarper_portalX'} = $x;
		$config{'partyWarper_portalY'} = $y;
		writeDataFileIntact("control/config.txt", \%config);
	}
	print "Portal setup at: $config{'partyWarper_portalX'}, $config{'partyWarper_portalY'}\n";
}
...
sub pw_auth {
	...
	my $flag = shift;
	if ($flag) {
		print "Authorized user '$user' for portal\n";
	} else {
		print "Revoked portal privilages for user '$user'\n";
	}	
	$partyWarperAuth{$user} = $flag;
	writeDataFile("control/warperAuth.txt", \%partyWarperAuth);
}

sub pw_switch {
	my $slot = shift;
	$config{'partyWarper_slot_current'} = $slot;
	writeDataFileIntact("control/config.txt", \%config);
	print "Portal set to open to: $config{'partyWarper_slot_$slot'}\n";
}
...
sub encrypt {
	...
	if ($config{'encrypt'} == 1 && $conState >= 5) {
		...
		{
			use integer;
			$temp = ($encryptVal * $encryptVal * 2483);
		}
		$temp = ~(~($temp));
		$temp = $temp % 17;
		$mask[$temp] = 1;
...
sub sendWarpCast1 {
	...
	my $x = shift;
	my $y = shift;
	my $msg = pack("C*", 0x16,0x01, 0x0A, 0x00, 0x1B, 0x00) . pack("S*", $x, $y);
	encrypt($r_socket, $msg);
}
...
sub chatLog {
	...
	if( $type eq "c" || $type eq "e"){
		...
		open CHAT, ">> $servers[$config{'server'}]{'name'}PrivateChat.txt";
	} elsif ( $type eq "s") {
		open CHAT, ">> $servers[$config{'server'}]{'name'}GMMessage.txt";
	} elsif ( $type eq "p") {
		open CHAT, ">> $servers[$config{'server'}]{'name'}PartyChat.txt";
	} elsif ( $type eq "g") {
		open CHAT, ">> $servers[$config{'server'}]{'name'}GuildChat.txt";
	} elsif ( $type eq "i") {
		open CHAT, ">> $servers[$config{'server'}]{'name'}GetItem.txt";
	} else {
		open CHAT, ">> $servers[$config{'server'}]{'name'}Etc.txt";
	}
...
sub chatLog_clear { 
	if (-e "$servers[$config{'server'}]{'name'}Chat.txt") { unlink("$servers[$config{'server'}]{'name'}Chat.txt"); } 
	if (-e "$servers[$config{'server'}]{'name'}PrivateChat") { unlink("$servers[$config{'server'}]{'name'}PrivateChat.txt"); } 
	if (-e "$servers[$config{'server'}]{'name'}GMMessage.txt") { unlink("$servers[$config{'server'}]{'name'}GMMessage.txt"); } 
	if (-e "$servers[$config{'server'}]{'name'}PartyChat.txt") { unlink("$servers[$config{'server'}]{'name'}PartyChat.txt"); } 
	if (-e "$servers[$config{'server'}]{'name'}GuildChat.txt") { unlink("$servers[$config{'server'}]{'name'}GuildChat.txt"); } 
	if (-e "$servers[$config{'server'}]{'name'}GetItem.txt") { unlink("$servers[$config{'server'}]{'name'}GetItem.txt"); } 
	if (-e "$servers[$config{'server'}]{'name'}Etc.txt") { unlink("$servers[$config{'server'}]{'name'}Etc.txt"); } 
}
...
sub connectPatchText {
	#return conState
	my ($useragnet, $URL, $request);
	my @URLs = qw(patch_allow.txt
			   notice_patch.txt
			   patch.txt
			  );
	...
	foreach $URL (@URLs) {
		...
		$request = $useragent->request("http://".$config{'patch_host'}.$config{'patch_dir'} . $URL);
	...
	if (!$monsters{$ID}{'judge'} && $monsters{$ID}{'dmgTo'} ne $monsters{$ID}{'dmgFromYou'}){
		...
		if($monsters{$ID}{'dmgFrom'} == 0 && $monsters{$ID}{'missedYou'} == 0){
			if ($config{'useDetection'} eq 1) {
				sendResponse(\$remote_socket, 'sorryWord');
			} elsif ($config{'useDetection'} > 1) {
...
sub sendResponse {
	...
	my $resType = shift;
	srand(time * rand(3));
	my $i = rand scalar(@$resType);
	chomp $$resType[$i];
	sendMessage($r_socket, "c", $$resType[$i]);
}
...
sub RecordPlayerData{
	...
	push(@temp,"$players{$ID}{'name'},".getHex($ID).",$players{$ID}{'party'}{'name'},$players{$ID}{'guild'}{'name'},$players{$ID}{'guild'}{'men'}{$players{$ID}{'name'}}{'title'},$players{$ID}{'jobID'},$players{$ID}{'sex'},","[".getFormattedDate(int(time))."]\n");
	truncate(FILE, 0);
	seek(FILE, 0, 0);
	print FILE @temp;
	close(FILE);
...
sub sendIgnoreall {
	...
	encrypt($r_socket, $msg);
	print "**** 全キャラクターからの耳打ちが$statになりました。 ****\n";
}
...
sub modifingName {
	...
	if (!$$r_hash{'type_equip'} || (!$$r_hash{'attribute'} && !$$r_hash{'refined'} && !$$r_hash{'star'} && !$$r_hash{'card'}[0])) {
		$$r_hash{'modified_name'} = $$r_hash{'name'};
	} else {
		if ($$r_hash{'card'}[0] == 0xff) {
			$$r_hash{'attribute'} = $$r_hash{'card'}[1] & 0x00ff;
			$$r_hash{'star'} = ($$r_hash{'card'}[1] & 0xff00) / 0x500;
			for($i = 0; $i < 4; $i++){
				$$r_hash{'card'}[$i] = 0;
			}
		}
		...
		$$r_hash{'modified_name'} = $modified.$$r_hash{'name'};
		if ($postmodify) {
			$$r_hash{'modified_name'} .= $postmodify;
		}
	}
}
